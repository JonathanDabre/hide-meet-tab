...
document.getElementById('toggleCamera').addEventListener('click', () => {
  document.getElementById('toggleCamera').style.backgroundColor = '#FF5733'; // Example red color
   // After 10 seconds, revert the color back to its original state
  setTimeout(() => {
    document.getElementById('toggleCamera').style.backgroundColor = ''; // Revert to original color
  }, 2000); // 10 seconds in milliseconds
          ...
          chrome.scripting.executeScript(
              ...
              () => {
                  if (chrome.runtime.lastError) {
                      ...
                      console.log("Script injected, sending toggleCamera message...");
                      chrome.tabs.sendMessage(tabs[0].id, { action: "toggleCamera" }, (response) => {
                          if (response && response.status === "done") {
